...
const NewTableCreation = ({ open, setOpen, getTableData }: NewTableCreationProps) => {
  ...
  const getDeviceData = async () => {
    ...
    try {
      const response = await axios.get(`${apiBaseUrl}/devices/byStore/667e3c007e2ed9e64a9136be`, {
        headers: { 'auth-token': token }
      })
      if (response && response.data) {
        const deviceIds = response.data.map((data: Record<string, any>) => {
          if (data.deviceId) {
              ...
              <Controller
                ...
                render={({ field: { value, onChange } }) => (
                  <TextField
                    ...
                    value={value}
                    onChange={onChange}
                    {...(errors.tableName && { error: true, helperText: errors.tableName.message })}
                  />
                )}
              />
            </Grid>
                ...
                render={({ field: { value, onChange } }) => (
                  <TextField
                    ...
                    {...(errors.minuteWiseRules?.dayUptoMin && {
                      error: true,
                      helperText: errors.minuteWiseRules?.dayUptoMin?.message
                    })}
                  />
                )}
              />
                ...
                render={({ field: { value, onChange } }) => (
                  <TextField
                    ...
                    {...(errors.minuteWiseRules?.dayMinAmt && {
                      error: true,
                      helperText: errors.minuteWiseRules?.dayMinAmt?.message
                    })}
                  />
                )}
              />
                ...
                render={({ field: { value, onChange } }) => (
                  <TextField
                    ...
                    {...(errors.minuteWiseRules?.dayPerMin && {
                      error: true,
                      helperText: errors.minuteWiseRules?.dayPerMin?.message
                    })}
                  />
                )}
              />
