...
const PurchaseReportTable = ({
  ...
  data: PurchaseReportTableDataType[]
  ...
  // const getPaymentMethod = (rowData: PurchaseReportTableDataType): string => {
  //   if (!rowData.received) {
  //     return 'DUE'
  //   }
  ...
  const columns = useMemo<ColumnDef<PurchaseReportTableDataType, any>[]>(
    () => [
      columnHelper.accessor('productName', {
        header: 'Product Name',
        cell: ({ row }) => <Typography color='text.primary'>{row.original.productName}</Typography>
      }),
      ...
      columnHelper.accessor('purchaseDate', {
        ...
        cell: ({ row }) => (
          <Typography color='text.primary'>
            {DateTime.fromISO(row.original.purchaseDate).toFormat('dd LLL yyyy hh:mm:ss a')}
          </Typography>
        )
      }),
      ...
      columnHelper.accessor('sku', {
        header: 'SKU',
        cell: ({ row }) => <Typography color='text.primary'>{row.original.sku || 0}</Typography>
      }),
      columnHelper.accessor('quantity', {
        header: 'Quantity',
        cell: ({ row }) => <Typography color='text.primary'>{row.original.quantity || 0}</Typography>
      }),
      columnHelper.accessor('purchasePrice', {
        header: 'Purchase Price',
        cell: ({ row }) => <Typography color='text.primary'>{`₹${row.original.purchasePrice || 0}`}</Typography>
      }),
      columnHelper.accessor('salePrice', {
        header: 'Sale Price',
        cell: ({ row }) => <Typography color='text.primary'>{`₹${row.original.salePrice || 0}`}</Typography>
      }),
      columnHelper.accessor('tax', {
        header: 'Tax',
        cell: ({ row }) => <Typography color='text.primary'>{row.original.tax || 0}</Typography>
      }),
      columnHelper.accessor('poValue', {
        header: 'Purchase Value',
        cell: ({ row }) => <Typography color='text.primary'>{row.original.poValue || 0}</Typography>
      })
    ],
    // eslint-disable-next-line react-hooks/exhaustive-deps
    []
  ...
  const table = useReactTable({
    data: purchaseData as PurchaseReportTableDataType[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
