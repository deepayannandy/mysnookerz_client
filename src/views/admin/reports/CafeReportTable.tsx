...
const columnHelper = createColumnHelper<CafeReportDataType>()
...
const CafeReportTable = ({
  data,
  getReportData
}: {
  data: CafeReportDataType[]
  ...
  const getPaymentMethod = (rowData: CafeReportDataType): string => {
    if (!rowData.paid) {
      return 'DUE'
    }
  ...
  const columns = useMemo<ColumnDef<CafeReportDataType, any>[]>(
    () => [
  ...
  const table = useReactTable({
    data: cafeData as CafeReportDataType[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
...
export default CafeReportTable
